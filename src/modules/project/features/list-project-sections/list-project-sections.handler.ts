...
@Injectable()
export class ListProjectSectionsHandler {
  constructor(
  ...
  async execute(actorUserId: string, projectId: string): Promise<ProjectSection[]> {
    const project = await this.projectRepository.findById(projectId);
    if (!project) throw new ProjectNotFoundError(projectId);
    await this.projectAccess.ensureCanViewProject(actorUserId, project);
    return this.sectionRepository.listByProject(project.id);
  }
}
